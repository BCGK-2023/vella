...
The graph projection depends DOWNWARD only and through the published front door.
This test walks every ``*.py`` under ``src/vella/graph``, parses it with ``ast``,
and asserts:
...
* no import targets a private ``vella.core._*`` symbol or submodule (the graph
  uses only ``vella.core``'s public surface).
...
_SRC = Path(__file__).resolve().parent.parent / "src" / "vella" / "graph"
...
def test_no_private_core_imports() -> None:
    ...
    Mirrors :func:`test_no_private_runtime_imports` for the core layer: the graph
