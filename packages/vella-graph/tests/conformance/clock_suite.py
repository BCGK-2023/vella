"""Adapter-agnostic conformance suite for the :class:`~vella.graph.Clock`.
...
``clock_factory`` returning a clock that also exposes an awaitable ``advance(dt)``
(the test-driving seam). The reference :class:`~vella.graph.ManualClock` is bound
in ``tests/test_clock.py``.

Re-declared from ``vella.reconciler``'s suite rather than imported — the graph
cannot depend on the reconciler (a shared testing-utils package is a v0.1
follow-up). No async plugin is required: each case is an ``async def`` coroutine
driven by a sync wrapper via ``asyncio.run``, with a bounded ``asyncio.wait_for``
backstop so a regression manifests as a failure, never a hang.
"""
