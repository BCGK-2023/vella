...
The reconciler ships exactly four runtime dependencies — pydantic,
typing_extensions, vella-core, and vella-runtime — and nothing more. A stray
dependency added to ``[project].dependencies`` (rather than the dev extras) fails
here, keeping the shipped footprint deliberate and depending strictly downward
(runtime, core), never up.
...
def test_reconciler_dependencies_are_exactly_four() -> None:
