#!/usr/bin/env python
"""
Agent public-surface breaking-change tripwire.

Snapshots the agent cognition core's public surface into
``schema/agent_surface.json`` and fails ``--check`` on undeclared drift
...
* ``models`` — each exported ``BaseModel``'s field name -> JSON-schema ``type``
  (so e.g. a future canonical-turn ``Message`` freezes its field shape; a field
...
* ``literals`` — each exported ``Literal`` alias's sorted allowed values, AND each
  ...
  ``planning``'s ``{"off","single","replan_on_failure"}`` value set is frozen rather
  than erased to a bare ``"string"`` by the JSON-schema ``type`` — mirrors the
  runtime/graph surface scripts' handling of ``Literal`` values).
* ``verbs`` — public method signatures of exported coordination classes (the
  interpreter/provider/invoker contract — ``run`` / ``turn`` / ``invoke`` and
  friends as they land, plus constructors), rendered via ``inspect.signature``
  (deterministic, free of the import-order nondeterminism ``typing.get_type_hints``
  can introduce) — mirrors how the graph surface script freezes its query verb
  signatures. At M0 ``__all__`` is empty, so this is ``{}``; it populates as the
  surface grows.

Field/method-name lists are sorted — the agent sorts its OWN derived structures,
never core/runtime/graph model fields. Everything is emitted with
``sort_keys=True`` — set-derived ordering must never leak into the serialized
artifact.

Usage:
    python scripts/export_agent_surface.py            # (re)write the baseline
    python scripts/export_agent_surface.py --check     # fail on undeclared drift
"""
...
import vella.agent as agent  # noqa: E402

ROOT = Path(__file__).resolve().parent.parent
BASELINE = ROOT / "schema" / "agent_surface.json"
...
def _literal_fields(model: type[BaseModel]) -> dict[str, list[Any]]:
    ...
    a value set is frozen against silent additions/removals (mirrors the
    runtime/graph scripts' ``Literal`` handling).
    """
...
def _method_signatures(cls: type) -> dict[str, str]:
    ...
    own-declared methods is what lets this freeze the interpreter/provider/invoker
    verb contract of a ``BaseModel`` subclass without dragging in pydantic's
    inherited ``model_*`` machinery, whose set is pydantic-version-sensitive.
    Annotations render via ``inspect.signature``'s own string form — deterministic
    and free of the import-order nondeterminism ``typing.get_type_hints`` can
    introduce. Mirrors the runtime/graph surface scripts.
    """
...
def generate() -> dict[str, Any]:
    ...
    signatures. At M0 ``__all__`` is empty, so every section is empty.
    """
    ...
    for name in exported:
        ...
        if isinstance(obj, type) and issubclass(obj, BaseException):
            ...
            # A BaseModel that also declares public methods freezes those
            # own-declared signatures too — the JSON-schema/field view captures only
            # data, not the verb contract.
            own_verbs = {
                k: v
            ...
            # A coordination/Protocol class without a model/error base: freeze its
            # public method signatures (the interpreter/seam contract as it grows).
...
def main() -> int:
    """Write or check the agent-surface baseline; return a process exit code."""
    ...
    if _serialize(surface) != _serialize(committed):
        print("Agent public-surface drift:", file=sys.stderr)
        ...
        print(
            "If intentional, re-run scripts/export_agent_surface.py and commit "
            "schema/agent_surface.json.",
            file=sys.stderr,
        )
        return 1
